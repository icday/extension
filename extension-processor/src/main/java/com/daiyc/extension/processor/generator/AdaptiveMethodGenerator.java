...
public class AdaptiveMethodGenerator extends BaseMethodGenerator {
    ...
    public boolean preGenerate(GenerateContext ctx) {
        ...
        if (matchType == BY_TYPE) {
            ...
            DeclaredType enumType = toEnumMeta.getEnumType();
            switch (toEnumMeta.getMatchType()) {
                ...
                case BY_FIELD:
                    ...
                    builder.initializer("$T.byField($T.class, $T::$L)", EnumMatcher.class, enumType, field.getSimpleName());
                    break;
                case BY_ORDINAL:
    ...
    protected FieldSpec.Builder newMatcherFieldBuilder(Class<?> type, TypeName fromType) {
        return FieldSpec.builder(
                ParameterizedTypeName.get(ClassName.get(type), fromType, ClassName.get(String.class))
